...
static const uint16_t mPanId = 0x3333;
...
static uint8_t App_SendAssociateResponse(nwkMessage_t *pMsgIn, uint8_t appInstance)
{
    ...
    int8_t idx = FindNodeByExtendedAddress(extAddr);
    uint16_t assignedShortAddr = 0;
    ...
    {
        // Generar nueva dirección corta
        uint16_t newShortAddr = 0x1000 + mNumAssociatedNodes;
...
static void App_HandleMcpsInput(mcpsToNwkMessage_t *pMsgIn, uint8_t appInstance)
{
  ...
  case gMcpsDataInd_c:
    /* The MCPS-Data indication is sent by the MAC to the network
	  ...
	  Serial_Print(interfaceId, ", Sender Addr: 0x", gAllowToBlock_d);
	  Serial_PrintHex(interfaceId, pMsgIn->msgData.dataInd.srcAddr, 2, TRUE);
      ...
      // Imprimir mensaje de actualización del timer
